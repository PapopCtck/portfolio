...
const useResponsiveDimension = (
  ...
  useEffect(() => {
    ...
    const calc = () => {
      ...
      const responsiveConfig = config as Pick<
        GradualBlurProps,
        DimensionKey | ResponsiveDimensionKey
      >;
      const mobileKey = `mobile${k}` as ResponsiveDimensionKey;
      const tabletKey = `tablet${k}` as ResponsiveDimensionKey;
      const desktopKey = `desktop${k}` as ResponsiveDimensionKey;
      if (w <= 480 && responsiveConfig[mobileKey]) {
        v = responsiveConfig[mobileKey];
      } else if (w <= 768 && responsiveConfig[tabletKey]) {
        v = responsiveConfig[tabletKey];
      } else if (w <= 1024 && responsiveConfig[desktopKey]) {
        v = responsiveConfig[desktopKey];
      }
      setVal(v);
    };
...
const GradualBlurExport = GradualBlurMemo as GradualBlurComponent;
GradualBlurExport.PRESETS = PRESETS;
GradualBlurExport.CURVE_FUNCTIONS = CURVE_FUNCTIONS;
